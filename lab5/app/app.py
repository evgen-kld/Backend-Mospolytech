...
@app.before_request
def log_visit_info():
    if request.endpoint == 'static' or request.path == '/favicon.ico' or request.args.get('download_csv'):
        return None
...
def update(user_id):
    ...
    with mysql.connection.cursor(named_tuple=True) as cursor:
        try:
            cursor.execute((f"UPDATE users SET {','.join(['{0}=%({0})s'.format(k) for k, _ in params.items() if k != 'id'])} WHERE id = %(id)s;"), params)
            mysql.connection.commit()
        except connector.Error:
            flash('Введены некоректные данные. Ошибка сохранения', 'danger')
